...
def generate_delivery_slots(neighbourhoods, distances, capacity):
    ...
    path_counter = count(start=1)  # Counter to keep track of the path number
    ...
    while unvisited_neighbourhoods:
        ...
        for nearest_node in sorted(unvisited_neighbourhoods, key=lambda x: distances[current_node]['distances'][int(x[1:])]):
            order_quantity = int(neighbourhoods[nearest_node]["order_quantity"])

            if current_capacity + order_quantity <= capacity:
                current_slot.append(nearest_node)
                current_capacity += order_quantity
            else:
                yield {f"path{next(path_counter)}": current_slot}
...
def main():
    with open('C:\\21pw14 - hack\\level1a.json') as file:
        data = json.load(file)
    ...
    output = {"v0": {}}
    path_counter = count(start=1)

    for i, slot in enumerate(delivery_slots):
        path_key = f"path{next(path_counter)}"
        path = slot[path_key]
        output["v0"][path_key] = {i: f"\"{node}\"" for i, node in enumerate(path)}

    with open('C:\\21pw14 - hack\\output_level1a.json', 'w') as f:
